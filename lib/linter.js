...
import { validateUnsafeInterpolation } from "./rules/unsafe-interpolation.js";
import { validateDependencyVersions } from "./rules/dependency-version.js";
import { validatePackageMetrics } from "./rules/package-metrics.js";
...
export async function lint(dir, options = {}) {
  ...
  for (const filePath of aux4Files) {
    ...
    if (parsed) {
      const ctx = { source: content, filePath, strict: options.strict };
      ...
      issues.push(...validateUnsafeInterpolation(parsed, filePath, ctx));
      issues.push(...validateDependencyVersions(parsed, filePath, ctx));
      issues.push(...validatePackageMetrics(parsed, filePath, ctx));
      if (options.resolve) {
