...
export function findProfileLine(source, profileName) {
  if (typeof profileName !== "string" || profileName.length === 0) return null;
...
export function findCommandLine(source, profileName, commandName) {
  // A nameless (or non-string) command/profile has no locatable "name" in the
  // source. Bail out with no line rather than building a regex from undefined —
  // a malformed node must never take down the whole lint run.
  if (typeof profileName !== "string" || profileName.length === 0) return null;
  if (typeof commandName !== "string" || commandName.length === 0) return null;
...
/**
 * Find the line of a hook by its command pattern.
 * Hooks have no "name" field, so they are located by "command": "<pattern>"
 * within the top-level "hooks" array.
 */
export function findHookLine(source, command) {
  const hooksMatch = /"hooks"\s*:\s*\[/.exec(source);
  if (!hooksMatch) return null;

  const afterHooks = source.substring(hooksMatch.index);
  const commandPattern = new RegExp(`"command"\\s*:\\s*"${escapeRegex(command)}"`);
  const commandMatch = commandPattern.exec(afterHooks);
  if (!commandMatch) {
    const lines = source.substring(0, hooksMatch.index).split("\n");
    return lines.length;
  }

  const totalOffset = hooksMatch.index + commandMatch.index;
  const lines = source.substring(0, totalOffset).split("\n");
  return lines.length;
}

function escapeRegex(str) {
  // Defensive: never throw on a non-string (e.g. an undefined name from a
  // malformed node). A crashing linter hides every other finding in the
  // directory, which is worse than missing one line number.
