  ...
  for (const profile of parsed.profiles) {
    ...
    for (const command of profile.commands) {
      ...
      for (const instruction of command.execute) {
        ...
        validateFnArgs(instruction, VALUES_FN, declaredVars, declaredRoots, true, profile.name, command.name, filePath, line, reported, issues);

        // param() / params() — supports :alias and ** suffixes, no *
        validateFnArgs(instruction, PARAMS_FN, declaredVars, declaredRoots, false, profile.name, command.name, filePath, line, reported, issues);

        // object() — no *
        validateFnArgs(instruction, OBJECT_FN, declaredVars, declaredRoots, false, profile.name, command.name, filePath, line, reported, issues);
      }
    }
  }
...
function validateFnArgs(instruction, pattern, declaredVars, declaredRoots, allowStar, profileName, commandName, filePath, line, reported, issues) {
  pattern.lastIndex = 0;
  let match;
