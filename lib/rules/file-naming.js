...
function validateFilename(basename, fullPath, dirType, expectedNames, aux4FilePath) {
  ...
  if (singleUnderscoreMatches.length > 0 && doubleUnderscoreCount === 0) {
    ...
    if (parts.length >= 2) {
      ...
      // Only warn if the suggested name matches or is close to an expected name
      // or if there's no expected name with single underscore (meaning it's not a
      // legitimate colon replacement)
      if (expectedNames.has(suggested) || !expectedNames.has(basename)) {
        issues.push({
          rule: "file-naming",
          severity: "error",
