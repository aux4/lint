...
export function validateReferenceIntegrity(parsed, filePath, ctx) {
  ...
  for (const profile of parsed.profiles) {
    ...
    for (const command of profile.commands) {
      // A nameless command is unreachable and already reported by the
      // command-structure rule. Skip it here so we neither crash on the missing
      // name nor emit misleading "Command 'undefined'" reference findings.
