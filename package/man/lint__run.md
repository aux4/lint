...
- **Variable references** — `${var}` in execute arrays should match declared variables; built-in variables (`${response}`, `${item}`, `${index}`, `${packageDir}`, `${aux4HomeDir}`, `${configDir}`, `${__callerPackage}`) are excluded; config-bound commands are skipped. Dot notation and array indexing are supported (`${obj.items[0].name}`), and shell parameter expansions like `${HOME:-/tmp}` are ignored
...
- **Parameter functions** — variables in `value()`, `values()`, `param()`, `params()`, `object()` must be declared unless core injects them as built-ins; `*` is only valid in `value()` and `values()`; `var*`/`var**` requires `multiple: true`; `param(name:alias)` and `param(name**)` are supported
...
- **Dependency versions** — the version token of every `dependencies` entry (`scope/name@<version>`) is validated. A complete version (`1.2.3`), a prerelease (`1.0.0-local`), `latest` and an omitted version are accepted as-is; anything carrying npm range syntax (`^`, `~`, `>=`, `>`, `<=`, `<`, `=`, `!=`, x-ranges, `*`, hyphen ranges, `||`, space-separated AND) is parsed with the same grammar the package manager uses at install time, so a malformed range is reported before publish instead of at a user's install. Anything that is neither — `banana`, `v1.2.3`, `1.2.3.4`, or a bare partial like `2.0` — is reported, because it can never resolve to a published version. A bare partial is worth calling out: the package manager parses it as an *exact* reference rather than a range (deliberate backward compatibility), but nothing is published under the literal name `2.0`, so the dependency fails to install; write `~2.0` or `2.0.x` instead
